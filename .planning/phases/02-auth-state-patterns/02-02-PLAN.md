...
must_haves:
  truths:
    ...
    - "submit.js Auth.init().then() call has a clarifying comment explaining the double-init is intentional for .then() chaining (submit.html already calls Auth.init() inline)"
...
<task type="auto">
  ...
  **A. profile.js — Replace ad-hoc error/loading display with Utils helpers**
  ...
  ```
  Note: "Try again" navigates to voices.html since reloading won't fix a missing ID.
