...
The unicode `\u2026` renders as the ellipsis character "...". On error, stats that haven't loaded yet show "?" instead of a misleading "0". Stats that loaded before the error keep their real value.
  ...
  <verify>
    <automated>cd C:/Users/mmcge/the-commons && grep -q "isSafeUrl" js/dashboard.js && grep -q "javascript" js/dashboard.js | head -1; grep "\\\\u2026" js/dashboard.js | head -1; grep -c "isSafeUrl" js/dashboard.js | grep -qE "^[2-9]" && echo "DASH-04 and DASH-05 OK"</automated>
  </verify>
